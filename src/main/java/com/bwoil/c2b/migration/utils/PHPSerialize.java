 ...
 *
 * @author wuzf(吴忠伏)
 * @ClassName: PHPSerialize
 * @date 2017年8月8日 上午11:56:30
 * @updateTime 2017年11月2日 下午6:30:13
 */
...
public class PHPSerialize {
        ...
        if (obj instanceof java.io.Serializable) {
            ...
            if (obj instanceof Serializable) {
                ...
                for (int i = 0, len = f.length; i < len; i++) {
                    ...
                    hv = serialize(stream, object, ht, hv, charset);
                }
                stream.write(__RightB);
            }
        } else {
    ...
    public PHPValue unserialize(String s) {
        try {
            sbb = ByteBuffer.wrap(s.getBytes("utf-8"));
        } catch (Exception ex) {
            sbb = ByteBuffer.wrap(s.getBytes());
        }
    ...
    private PHPValue getNext() {
        ...
        while (sbb.position() < sbb.capacity()) {
            c = sbb.get();
            ...
            if (what.equals("array_length")) {
                if (c == ':') {
                    len = Integer.parseInt(new String(tmp.array()));
                    tmp = ByteBuffer.allocate(len);
                    what = "array_value";
                }
                if (len == -1) {
                    if (tmp.position() == tmp.capacity()) {
                        byte[] tmpBytes = tmp.array();
                        tmp = ByteBuffer.allocate(tmpBytes.length + 1);
                        tmp.put(tmpBytes);
                    }
                    tmp.put(c);
                }
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (hashValue.toObject() instanceof Map) {
                        hash.put(hashKey.value, hashValue.toMapObject());
                    } else {
                        hash.put(hashKey.value, hashValue.toString());
                    }
                }
                value = hash;
                ...
                if (c == ';') {
                    value = new String(tmp.array()).trim();
                    what = "set";
                } else {
                    if (tmp.position() == tmp.capacity()) {
                        byte[] tmpBytes = tmp.array();
