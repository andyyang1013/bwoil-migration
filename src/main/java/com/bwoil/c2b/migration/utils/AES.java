...
public class AES {
    ...
    public static String encrypt(String sSrc) {
        if (org.springframework.util.StringUtils.isEmpty(sSrc)) {
            return null;
        }
        try {
		...
		/*if (sSrc.length() % 16 == 0) {
            ...
            return AES.parseByte2HexStr(encrypted);
        } catch (Exception e) {
    ...
    public static String decryptStr(String hexStr) {
        if (org.springframework.util.StringUtils.isEmpty(hexStr)) {
            return null;
        }
        return decrypt(parseHexStr2Byte(hexStr));
    }

    public static String decrypt(byte[] sSrc) {
        sKey = paddingKey(sKey);
    ...
    public static void main(String[] args) throws Exception {
        String encrypt = AES.encrypt("6214867827116808");
        String parseByte2HexStr = AES.parseByte2HexStr(encrypt.getBytes());
